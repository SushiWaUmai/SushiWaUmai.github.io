...
function drawConfiguration(configuration, x, y) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        let value1 = createVector(
            heightMap[x + positions[range1.x].x][y + positions[range1.x].y],
            heightMap[x + positions[range1.y].x][y + positions[range1.y].y]
        );
        let value2 = createVector(
            heightMap[x + positions[range2.x].x][y + positions[range2.x].y],
            heightMap[x + positions[range2.y].x][y + positions[range2.y].y]
        );
        ...
        let point1 = vecLerp(
            positions[x1].toPVector(),
            positions[y1].toPVector(),
            (value1.x + value1.y) / 2
        );
        let point2 = vecLerp(
            positions[x2].toPVector(),
            positions[y2].toPVector(),
            (value2.x + value2.y) / 2
        );
        ...
        vecLine(
            vecGridToPixel(point1.add(posVec)),
            vecGridToPixel(point2.add(posVec))
        );
    }
}
...
function initHeightMap() {
    ...
    for (let x = 0; x < mapWidth; x++) {
        for (let y = 0; y < mapHeight; y++) {
            heightMap[x][y] = noise(
                (x / mapWidth) * xScale + xOffset,
                (y / mapHeight) * yScale + yOffset,
                zOffset
            );
        }
    }
...
function gridToPixel(x, y) {
    return createVector(
        x * xCellSize + xCellSize * 0.5,
        y * yCellSize + yCellSize * 0.5
    );
}
